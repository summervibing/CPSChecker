...
public interface MonitorHandler {
     ...
     * @return {@link Monitor} of the given {@link Player} or {@code null} if the player is not monitoring
     * anyone
     */
    @Nullable Monitor access(@NotNull Player player);

    /**
     * Checks if the given {@link Player} is currently monitoring any user.
     *
     * @param player {@link Player} to check monitoring status for
     * @return {@code true} if the player is monitoring, {@code false} otherwise
     */
    boolean isMonitoring(@NotNull Player player);

    /**
     * Retrieves all currently monitoring {@link Player Players} and their associated
     * {@link Monitor Monitors}.
     *
     * @return Unmodifiable {@link Map} of {@link Player Players} and their {@link Monitor Monitors}
     */
    @NotNull Map<Player, Monitor> monitors();

    /**
     * Starts monitor for given {@link Player} of the {@link User} with the given {@link UUID}.
     *
    ...
    @NotNull MonitorResult monitor(
            @NotNull Player player,
            @NotNull UUID uniqueId,
            @Nullable PatternType patternType
    );
     ...
     * @return {@link MonitorResult} of the operation.
     */
    @NotNull MonitorResult stop(@NotNull Player player);

    /**
     * Switches to {@link Monitor#nextClickType()} for the given {@link Player}.
     *
     * @param player {@link Player} to switch {@link ClickType} for
     * @return {@link MonitorResult} of the operation
     */
    @NotNull MonitorResult nextClickType(@NotNull Player player);

    /**
     * Switches to {@link Monitor#nextPatternType()} for the given {@link Player}.
     *
     * @param player {@link Player} to switch {@link PatternType} for
     * @return {@link MonitorResult} of the operation
     */
    @NotNull MonitorResult nextPatternType(@NotNull Player player);
